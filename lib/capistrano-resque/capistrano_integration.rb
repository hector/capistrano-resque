...
PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 #{workers_env} bundle exec rake environment resque:work"
                worker_id += 1
              end
            end
          end
...
PIDFILE=./tmp/pids/scheduler.pid BACKGROUND=yes #{scheduler_env} bundle exec rake resque:scheduler"
            end
