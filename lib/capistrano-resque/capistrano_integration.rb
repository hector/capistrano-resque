              ...
              workers.each_pair do |queue, number_of_workers|
                ...
                number_of_workers.times do
                  ...
                  run("cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE=\"#{queue}\" \
                   PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 #{workers_env} #{fetch(:bundle_cmd, "bundle")} exec rake resque:work >> #{shared_path}/log/resque.log 2>&1 &", 
                      :roles => role)
                  worker_id += 1
                end
              end
...
PIDFILE=./tmp/pids/scheduler.pid BACKGROUND=yes #{scheduler_env} bundle exec rake resque:scheduler >> #{shared_path}/log/resque_scheduler.log 2>&1 &"
            end
